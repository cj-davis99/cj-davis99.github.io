...
\usepackage{float}
\usepackage{tikz}
\usetikzlibrary{shapes, arrows, positioning, calc, graphs}
\usepackage{pgfplots}
...
\title{Lecture 2: Sequences, Continuity}
...
\begin{document}
\lhead{Math Camp \\ August 14, 2024} 
\rhead{Lecture 2 \\ \leftmark}
\maketitle
\tableofcontents
\newpage
\section*{Notation}
...
\newpage
\section{Sequences}
Formally, a (real-valued) sequence is a function from $\N$ to $\R^N$ where $N\in \N$; however, it's often simpler to think of sequences as a collection of elements of a set indexed by the natural numbers. We will not be terribly precise with notation,\footnote{We can also think formally of a sequence as an element of the infinite Cartesian product of $\R^N \times \R^N \times \cdots$. While tempting to define a sequence as a countable or finite subset of $\R^N$, sets have no notion of order; further, sequences can have repeated elements.} and denote sequences with elements in $S$ as $(x_m) \in S$.
\begin{definition}
  Let $(x_m) \in S$ be a sequence:
  \begin{enumerate}
    \item[a)] $(x_m)$ is \keyword{increasing} if $\forall m$ we have $x_m \le x_{m + 1}$; it is \textit{strictly} increasing if $x_m < x_{m + 1}$.
    \item[b)] $(x_m)$ is \keyword{decreasing} if $\forall m$ we have $x_m \ge x_{m + 1}$; it is \textit{strictly} decreasing if $x_m > x_{m + 1}$.
    \item[c)] $(x_m)$ is (strictly) \keyword{monotonic} if it is (strictly) increasing or decreasing .
    \item[d)] $(y_k)$ is a \keyword{subsequence} of $(x_m)$ if $\exists$ some strictly increasing sequence $(n_k) \in \N$ s.t. $y_k = x_{m_k}$.
    \item[e)] $(x_m)$ is bounded above, below, or bounded if $\{x_m\}_{m \in \mathbb{N}}$ is bounded above, below, or bounded (resp).
    \item[f)] $(x_m) \to +\infty$ if $\forall N > 0, ~~ \exists M$ s.t. $m \ge M \implies x_m \ge N$.  $(x_m) \to -\infty$ if $\forall N < 0, ~~ \exists M$ s.t. $m \ge M \implies x_m \le N$. If either property holds, we say the sequence $(x_m)$ \keyword{diverges to infinity}.
  \end{enumerate}
\end{definition}

\subsection{Convergence}
\begin{definition}\label{def:lecture2_convergence}
  A sequence $(x_m)$ \keyword{converges} to $x$ if $\forall \varepsilon > 0, ~~ \exists M \in \N$ s.t. $d(x_m, x) < \varepsilon$ whenever $m \ge M$. We denote this as $x_m \to x$ or $\lim_{m \to \infty} x_m = x$. Otherwise, we say that the sequence $(x_m)$ \keyword{diverges}. 
\end{definition}


We can visualize this idea in the figure below: $x_m$ is eventually contained within any $\varepsilon$-ball if $x$.\footnote{Recall $\varepsilon$-ball is the equivalent of a neighborhood in Euclidean space, even through here in two dimensions it's technically a $\varepsilon$-circle.} 
...
\end{claim}
\begin{proof}
     This is a consequence of the fact $d(x, y) \iff x = y$. Suppose $x_m \to x$ and $x_m \to y$. If $x \ne y$, then $d(x, y) > 0$. Take $\varepsilon = d(x, y)$; we know by the definition of convergence that there is some $M_x, M_y$ s.t. for $m \ge M = \max\left\{M_x, M_y\right\}$ we get
  \[
...
\begin{theorem}\label{thm:lecture2_closed_sequential}
  $S$ is closed $\iff$ for any $(x_m) \in S$ s.t. $x_m \to x$ for some $x \in \R^N$, we have that $x \in S$.
\end{theorem}
This is an equivalent definition of closedness: A set that ``contains all its limits.''
\begin{proof}
    See Subsection~\ref{ssub:sets_and_sequences_visualization_} for a visualization (try formally writing the proof yourself!).
\end{proof}
...
\begin{theorem}
  ...
  \begin{enumerate}
    \item[a)] $x \in \cl(S)$.
    \item[b)] $\forall \varepsilon > 0, B_{\varepsilon}(x) \cap S \ne \varnothing$.
    \item[c)] $\exists (x_m) \in S$ s.t. $x_m \to x$.
  \end{enumerate}
\end{theorem}
\begin{proof}
    We prove this equivalence by proving $b \implies c \implies a \implies b$.
    \begin{enumerate}
        \item[$(b\Rightarrow c)$] Take any $x$ such that condition $b$ holds. Define $(x_m)\in S$ such that $\forall m\in \N$
        \begin{equation*}
            x_m \in B_{1/m}(x)\cap S \neq \varnothing
        \end{equation*}
        Showing that $x_m \to x$ completes the proof. Fix any $\varepsilon > 0$. By the Archimedean Property (see Lecture 1 notes), we know that there exists an $M\in \N$ such that $0 < 1/M < \varepsilon$. Thus,
        \begin{equation*}
            d(x_m,x) < 1/m < \varepsilon
        \end{equation*}
        whenever $m\geq M$, as desired. 
        
        \item[$(c\Rightarrow a)$]
        Let $(x_m)$ be any convergent sequence in $S$ such that $x_m \to x$ (we know such a sequence exists because we are assuming condition $c$). We want to show that $x \in \cl(S)$. Since $S \subseteq \cl(S)$, we know that $(x_m) \in \cl(S)$. Moreover, we know that $\cl(S)$ is a closed set. By Theorem~\ref{thm:lecture2_closed_sequential}, it must be that $x\in \cl(S)$. 
        
        \item[$(a\Rightarrow b)$] Suppose there exists $\varepsilon > 0$ such that 
        \begin{equation*}
            B_\varepsilon(x) \cap S = \varnothing
        \end{equation*}
        We want to show that $x\not\in \cl(S)$. Since $B_\varepsilon(x)$ is open, we know that its complement is closed. Since $\cl(S)$ is a closed set, $\cl(S)\setminus B_\varepsilon(x)$ is a closed set. By definition, $S \subseteq \cl(S)$. Since $B_\varepsilon(x) \cap S = \varnothing$, it must also be that 
        \begin{equation*}
            S \subseteq \cl(S)\setminus B_\varepsilon(x)
        \end{equation*}
        Hence, $\cl(S) \subseteq \cl(S)\setminus B_\varepsilon(x)$ by the definition of closure. Since $x \in B_\varepsilon(x)$, we know that
        \begin{equation*}
            x\not\in \cl(S)\setminus B_\varepsilon(x) \implies x \not\in \cl(S)
        \end{equation*}
        as desired. 
    \end{enumerate}
\end{proof}
...
\begin{definition}
  ...
  \[
    \limsup_{m \to \infty} x_m = \lim_{m \to \infty} \left(\sup_{k \ge m} x_k\right)
  \]

  and
  \[
    \liminf_{m \to \infty} x_m = \lim_{m \to \infty} \left(\inf_{k \ge m} x_k\right)
  \]
\end{definition}
...
\subsection{Sets and Sequences (Visualization)}
\label{ssub:sets_and_sequences_visualization_}

The mathy sequel to Dungeons \& Dragons. In this section we will try to visualize the proof of Theorem~\ref{thm:lecture2_closed_sequential}. This should give some intuition for why convergence is related to sets being closed beyond the formality of the theorem.
\begin{itemize}
    ...
    \begin{figure}[H]
      ...
      \begin{tikzpicture}
        \begin{axis}[name=plot1
          ...
          \node [above] at (axis cs:5.25, 5.15) {$B_{1/m}(y) \cap  A \ne \varnothing ~~ \forall m$};
...
\begin{itemize}
  \item $\R$ is not enough: A ton of things will hold in one dimension that won't in general, and one-dimensional intuition can end up being misleading.

  \item $\R^3$ can be too much: I cannot draw 3D very easily and it is harder to visualie clearly (certainly $\R^N$ would be too many dimensions).

  \item $\R^2$ is a nice trade-off between rigor an intuition.  (I know we discuss properties in more general terms than in $\R^2$, but for intuition I think it's a great benchmark.)
\end{itemize}
...
What follows are some useful properties of convergent sequences sometimes referred to as ``limit laws'' in calculus textbooks. We leave the proof of these results as an exercise for the reader. 
\begin{theorem}
  Take any sequence $(x_m) \in \mathbb{R}^N$:
  \begin{enumerate}
    \item[a)] $x_m \to x \implies x_{m_k} \to x$ for all subsequences $(x_{m_k})$ of $(x_m)$.

    \item[b)] $x_m \to x \implies (x_m)$ is bounded. (Is the converse true? Can you prove your answer?\footnote{A counterexample is sufficient: $x_m = (-1)^m$ is bounded above by $1$ and below by $-1$ but does not converge.})

    \item[c)] If $x_m \le y_m \le z_m$ and $x_m, z_m \to x$ then $y_m \to x$.

    \item[d)] If $x_m \to 0$ and $(y_m)$ is bounded then $x_m \cdot y_m \to 0$.
  \end{enumerate}

  Let $x_m \to x$ and $y_m \to y$:
  \begin{enumerate}
    \item[e)] $c \cdot x_m \to c \cdot x$ for any $c \in \mathbb{R}$.

    \item[f)] $x_m \pm y_m \to x \pm y$.

    \item[g)] $x_m \cdot y_m \to x \cdot y$.

    \item[h)] $x_m / y_m \to x / y$ if $y \ne 0$.
  \end{enumerate}
\end{theorem}
...
\begin{theorem}\label{thm:lecture2_bounded_mono_converge}
  Let $(x_m) \in \R$. If $(x_m)$ is bounded and monotonic then $(x_m)$ converges.
\end{theorem}
\begin{proof}
    See question 6 of problem set 1. \textit{Hint}: If $(x_m)$ is bounded and increasing, then what should $(x_m)$ converge to? Claim 10 in Lecture 1 notes may be useful here...
\end{proof}

\begin{theorem}[Nested Intervals Theorem]\label{thm:lecture2_nested_interval}
  Let $I_m = [a_m,  b_m]$ s.t. $I_{m + 1} \subseteq I_m$.
  \begin{enumerate}
    \item[a)] $\cap_{m \in \N} I_m \ne \varnothing$
    \item[b)] If $b_m - a_m \to 0$ then $\cap_{m \in \N}$ is  a singleton.
  \end{enumerate}
\end{theorem}
...
\begin{proof}
  ...
  for all $m$. In other words, $a_m$ and $b_m$ are bounded and monotonic, so $a_m \to a$ and $b_m \to b$ for some $a, b$ by Theorem~\ref{thm:lecture2_bounded_mono_converge}. Further, since $a_m \le b_m$ for all $m$, we have that $a \le b$ (do you see why?\footnote{If $a > b$ then we will have that $a_m > b_m$ for some $m$. That is, pick $0 < \varepsilon < a - b$; we can then find $M_a, M_b$ s.t.
    \[
      a - \varepsilon / 2 < a_m < a + \varepsilon / 2
      \quad\quad
      b - \varepsilon / 2 < b_m < b + \varepsilon / 2
    \]

    whenever $m > \max\{M_a, M_b\}$. However, $\varepsilon < a - b$ gives
    \[
...
\begin{theorem}[Bolzano-Weierstrass]
  Every bounded sequence in $\R$ admits a convergent sub-sequence.
\end{theorem}

\begin{proof}
  This follows from Theorem~\ref{thm:lecture2_nested_interval} above---the trick is to construct the nested intervals, which we can do for bounded sequences. If $(x_m)$ is bounded, first define
  \[
    I_1 = [L, U] = [a_1, b_1]
  \]

  the interval with endpoints equal to the lower and upper bounds of $x_m$. Let $x_{m_1}$ the first element of the sub-sequence be any term of $x_m \in I_1$. Now take
  \[
  ...
  and let $I_2 = I_1^-$ if $\{x_m: m \in \N\} \cap I_1^+$ is non-finite and $I_2 = I_1^+$ otherwise. Let $a_2, b_2$ be the endpoints of $I_2$ and $x_{m_2}$ be any term of $x_m \in I_2$. We iterate this process: In general
  \[
  ...
  and $I_{k + 1} = I_k^-$ if $\{x_m: m \in \N\} \cap I_k^+$ is non-finite and $I_{n + 1} = I_k^+$ otherwise, with $x_{m_k}$ any element of $x_m \in I_k$. (What if both halves have a finite intersection?\footnote{Note that for $k > 1$, it must always be that either $I_k^+$ or $I_k^-$ have a non-finite intersection with $\{x_m: m \in \mathbb{N}\}$, since we chose $I_k$ to have a non-finite intersection.  The only way both halves will have a finite intersection is if $I_1$ is finite to begin with. However, this means that some $M$, $x_n = x_m$ whenever $n, m > M$, which means we have a convergent sub-sequence $x_{m_k} = x_{M + 1}$ with $m_k = M + k$.}) We have that
  \[
    a_1 \le a_{k - 1} \le a_k \le x_{m_k} \le b_k \le b_{k - 1} \le b_1
  \]

  Furthermore, $b_n - a_n = \dfrac{b_1 - a_1}{2^{n - 1}} \to 0$. Hence we can apply the Theroem~\ref{thm:lecture2_nested_interval}: $a_n \to a$ and $b_n \to b$ with $a = b$ implies $x_{m_k} \to a = b$.
\end{proof}

\subsection{Cauchy Sequences}
\label{ssub:cauchy_sequences}
In this section, we define a generalization of a convergent sequence. 
\begin{definition}
  ...
  \]
\end{definition}
Informally, a Cauchy sequence is any sequence whose terms get arbitrarily close together as we look further into the sequence. The following theorem states that every convergent sequence is a Cauchy sequence. 
...
\begin{theorem}
  If $(x_m)$ is a Cauchy sequence in $\R$ then $(x_m)$ converges.
\end{theorem}
\begin{proof}
    We will show this in $\R$: Cauchy sequences are bounded (why?), which means that there exist some sub-sequence $x_{m_k}$ that converges to some $x$. We show that this is also the limit for the sequence $x_m$. By the triangle inequality,
  \begin{equation*}
      d(x_m, x) \le d(x_m, x_{m_k}) + d(x_{m_k}, x)
  \end{equation*}
  Take any $\varepsilon > 0$. Since $x_{m_k} \to x$, we know there exists a $K\in \N$ such that
  \begin{equation*}
      k > K \implies d(x_{m_k}, x) < \frac{\varepsilon}{2}
  \end{equation*}
  Since $(x_m)$ is Cauchy, we also know that there exists some $M \in \N$ such that 
  \begin{equation*}
      m,k > M \implies d(x_m, x_{m_k}) < \frac{\varepsilon}{2}
  \end{equation*}
  (note that $m_k \geq k$, so $k > M \implies m_k > M$). Letting $N=\max\{K,M\}$, we have that for any $m,k > N$
  \begin{equation*}
      d(x_m, x) \leq d(x_m, x_{m_k}) + d(x_{m_k}, x) < \varepsilon/2 + \varepsilon/2 = \varepsilon
  \end{equation*}
  This proof should generalize to $\R^N$ if you argue that along each coordinate, $(x_m)$ is bounded and then use that to find a candidate limit $x$. Then the identical argument goes through (note it used generic properties of the distance rather than anything specific to $\R$).
\end{proof}
...
\begin{proposition}\label{prop:IVT_lemma}
  Let $\varphi: \mathbb{R}^N \to \mathbb{R}$. If $\varphi$ is continuous then the sets 
  \begin{align*}
      \left\{x \in \mathbb{R}^N: \varphi(x) \ge \alpha\right\} \\
      \left\{x \in \mathbb{R}^N: \varphi(x)\le \alpha\right\}
  \end{align*}
  and are closed for all $\alpha \in \mathbb{R}$.
\end{proposition}

\begin{proof}
  Let $A = \left\{x \in \mathbb{R}^N: \varphi(x) \ge \alpha\right\}$ and $B = \mathbb{R}^N \setminus A$. If $B$ is open then $A$ is closed. Note
  \[
    B = \left\{
  ...
  therefore $B$ is open (for any point in $B$ $\exists \delta$-ball that is entirely contained in $B$). Thus $\mathbb{R}^N \setminus B = A$ is closed. The proof for the lower sets is analogous.
\end{proof}

\begin{theorem}
  Let $f: X \to Y$ and $g: f(X) \subseteq Y \to Z$ with $X, Y, Z \subseteq \R^N$. If $f, g$ are continuous then $(g \circ f): X \to Z$ is continuous.
\end{theorem}

\begin{theorem}
  Let $f: X \to \mathbb{R}$ and $g: X \to \mathbb{R}$ be continuous functions. Then
  \begin{enumerate}
    \item $h(x) = f(x) \pm g(x)$ is continuous.
...
% ---------------------------------------------------------------------
\subsection{Sequential and Open Set Characterizations of Continuity}
\label{sub:sequential_and_open_set_characterizations}

\begin{remark}
  Continuous functions don't map open sets to open sets! Consider $f(x) = x^2$. The image of $(-2, 2)$ is $[0, 2)$, which is not open.  Further, even if all maps of closed sets are closed, the function might not be continuous. For example, Consider a function that is $0$ if $x < 0$ and $1$ if $x \ge 1$. This has a jump at $1$, but $f([a, b])$ is either $\{0\}, \{1\},$ or $\{0, 1\}$, which are closed.
...
\begin{theorem}\label{thm:lecture2_continuity_open_closed}
  ...
  \begin{enumerate}
    ...
    \item[b)] If $O \subseteq \mathbb{R}^M$ is open, $f^{-1}(O)$ is open.

    \item[c)] If $S \subseteq \mathbb{R}^M$ is closed, $f^{-1}(S)$ is closed.

    \item[d)] For every $(x_m) \in \mathbb{R}^N$ s.t. $x_m \to x$ for some $x \in \mathbb{R}^N$, $f(x_m) \to f(x)$.
  \end{enumerate}
\end{theorem}

We give visual intuition for the proof in Section~\ref{ssub:sets_and_continuity_visualization_}. 

\subsubsection{Sequences, Sets, and Continuity (Visualization)}
\label{ssub:sets_and_continuity_visualization_}

We show Theorem~\ref{thm:lecture2_continuity_open_closed} using, again, drawings in $\mathbb{R}^2$.

\begin{proof}
  We cycle through the statements. If we show $a \implies b \iff c \implies d \implies a$ then we have shown they are equivalent.
  \begin{enumerate}
    \item $a \implies b$. First, it is a good idea to write down what you have and what you want to show:
      ...
      \item We show $b \iff c$. First, consider any closed set $S \in \mathbb{R}^M$, so $\mathbb{R}^M \setminus S$ is open; by premise, $f^{-1}(\mathbb{R}^M \setminus S)$ is also open, which means $f^{-1}(\mathbb{R}^M) \setminus f^{-1}(\mathbb{R}^M \setminus S) = f^{-1}(\mathbb{R}^M \setminus (\mathbb{R}^M \setminus S)) = f^{-1}(S)$ is closed. (The only sticking point here would be to show that for general subsets $A,B$ of the co-domain of any function $f$, $f^{-1}(A)\setminus f^{-1}(B) = f^{-1}(A\setminus B)$\footnote{
          Suppose $x\in f^{-1}(A)\setminus f^{-1}(B)$. If $x\in f^{-1}(B) \Leftrightarrow f(x)\in B$, then $x\not\in f^{-1}(A)\setminus f^{-1}(B)$, a contradiction. If $x \not\in f^{-1}(A) \Leftrightarrow f(x)\not\in A$, then $x\not\in f^{-1}(A)\setminus f^{-1}(B)$, a contradiction. Thus, $f(x)\in A\setminus B \Leftrightarrow x \in f^{-1}(A \setminus B)$, implying that $f^{-1}(A)\setminus f^{-1}(B)\subseteq f^{-1}(A \setminus B)$. We can follow a nearly identical argument to show set containment in the other direction (try it!), giving us the desired equality.
      }). Now consider open set $O \in \mathbb{R}^M$, so $\mathbb{R}^M \setminus O$ is open; by premise, $f^{-1}(\mathbb{R}^M \setminus O)$ is also closed, which means $\mathbb{R}^N \setminus f^{-1}(\mathbb{R}^M \setminus O) = f^{-1}(O)$ is open. You will notice this is an entirely analogous argument.
      ...
      \begin{itemize}
        ...
        We can see as $x_m$ becomes increasingly closer to $x$, $f(x_m)$ is always at least $\varepsilon$ away from $f(x)$. In other words, we have constructed a sequence $x_m \to x$ were $f(x_m) \not\to f(x)$, contradiction.
        \end{itemize}
    \end{enumerate}
\end{proof}
...
\begin{theorem}[Intermediate Value Theorem (IVT)]
  If $f: [a, b] \to \mathbb{R}$ is continuous in $[a, b]$ then for any $L$ between $f(a)$ and $f(b)$, 
  \[
    \exists c \in [a, b] \quad\text{s.t.}\quad f(c) = L
  \]
\end{theorem}
...
\begin{proof}
  WLOG, suppose $f(a) < f(b)$ (the equality case is trivial). Take any $L \in (f(a), f(b))$ and consider
  \[
    A = \{x \in [a, b]: f(x) \le L\}
  \]

  $A$ is bounded so the $\sup$ exists; let $c = \sup A$. For $\varepsilon_m = 1/m$ take $x_m \in (c - \varepsilon_m, c] \cap A$, so $x_m \to c$ and $f(x_m) \to f(c)$ (by continuity). Since $A$ is closed (see Proposition~\ref{prop:IVT_lemma}) and $x_m \in A$, $c \in A$ (by Theorem~\ref{thm:lecture2_closed_sequential}) and $f(c) \le L$. If $f(c) = L$ we are done; if $f(c) < L$ then consider
  \[
    B = \{x \in [c, b]: f(x) \ge L\}
  \]

  Note $B = (c, b]$ because $f(c) < L$, so $c \notin B$, and $c$ is an upper bound for $A$: For every $x \in (c, b]$ it cannot be $f(x) < L$ because then $x \in A$ and $x > c$, a contradiction, so $f(x) \ge L$. However, this is another contradiction because the upper set of a continuous function is closed, and $B = (c, b]$ is not closed. Hence $f(c) = L$. (If you take $\inf(B)$ you can follow the same logic to get $f(c) > L$; basically we're showing there's a discontinuity at $c$, which cannot happen for a continuous function.) (Another subtlety here is the case when $c = b$, but then $f(b) = f(c) < L < f(b)$ contradiction.)
\end{proof}

\section{Fun Remarks}
\label{sub:fun_remarks}

You can have a fun relaxing minute listening to Tom Lehrer's \href{https://www.youtube.com/watch?v=zxFCQplZgKI#t=2m35s}{\textit{There's a Delta for Every Epsilon}}:
\begin{quote}
